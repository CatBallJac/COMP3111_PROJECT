        ...
        protected void AlertValidator_ServerValidate(object source, ServerValidateEventArgs args)
        {
            ...
            if (lowalert.Checked)
            {
                if (decimal.TryParse(lowvalue.Text.Trim(), out lowalertMoney))
                {
                    ...
                    {
                        if(lowalertMoney.CompareTo((decimal)999.99) > 0)
                        {
                            args.IsValid = false;
                            AlertValidator.ErrorMessage = "The low alert value cannot exceed 999.99";
                            return;
                        }
                    }else if(securityType == "bond"||securityType=="unit trust")
                    {
                        if (lowalertMoney.CompareTo((decimal)9999.99) > 0)
                        {
                            args.IsValid = false;
                            AlertValidator.ErrorMessage = "The low alert value cannot exceed 9999.99";
                            return;
                        }
                    }
                }
            ...
            if (highalert.Checked)
            {
                if (decimal.TryParse(highvalue.Text.Trim(), out highalertMoney))
                {
                    ...
                    if (securityType == "stock")
                    {
                        if (highalertMoney.CompareTo((decimal)999.99) > 0)
                        {
                            args.IsValid = false;
                            AlertValidator.ErrorMessage = "The low alert value cannot exceed 999.99";
                            return;
                    ...
                    {
                        if (highalertMoney.CompareTo((decimal)9999.99) > 0)
                        {
                            args.IsValid = false;
                            AlertValidator.ErrorMessage = "The low alert value cannot exceed 9999.99";
                            return;
